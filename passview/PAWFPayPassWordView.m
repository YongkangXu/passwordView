...
-(UIView *)inputView{
    PAWFSafeKeyboard *keyboard = [[PAWFSafeKeyboard alloc]initWithType:PAWFSafeKeyboardTypeNormal];
    keyboard.useComfirnBtn = NO;
    keyboard.useDotKey = NO;
        ...
        if (basicText) {
            ...
            if (self.passWordString.length == self.passWordNumber) {
                if ([self.delegate respondsToSelector:@selector(passWordViewCompleteInput:AndPassWordString:)]) {
                    [self.delegate passWordViewCompleteInput:self AndPassWordString:self.passWordString];
                }
            }
            [self setNeedsDisplay];
        }
