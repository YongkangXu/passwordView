...
- (void)setCellContent:(NSIndexPath*)indexpath array:(NSArray*)array{
    if (indexpath.row == 11) {
        _imageView.image = [UIImage imageNamed:@"keybord_delete"];
    }else{
...
- (instancetype)initWithType:(PAWFSafeKeyboardType)type {
    ...
    if (self) {
        ...
        flow.itemSize = CGSizeMake(kScreenWidth / 3, self.height / 4);
        flow.sectionInset = UIEdgeInsetsZero;
        _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, self.height) collectionViewLayout:flow];
        ...
        [_delBtn setImage:[UIImage imageNamed:@"keybord_delete"] forState:UIControlStateNormal];
